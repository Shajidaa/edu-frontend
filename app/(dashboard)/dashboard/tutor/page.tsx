...
export default function TutorDashboard() {
    ...
    const tutorStats = [
        { name: 'Active Students', value: '24', icon: Users, color: 'bg-blue-500' },
        { name: 'Hours Taught', value: '156', icon: Clock, color: 'bg-green-500' },
        { name: 'Avg Rating', value: '4.8', icon: Star, color: 'bg-yellow-500' },
        { name: 'This Week', value: '18', icon: CalendarDays, color: 'bg-purple-500' },
    ];

    const upcomingSessions = [
        { id: 1, subject: 'Mathematics', student: 'Alice Johnson', time: 'Today, 2:00 PM', duration: '1 hour' },
        { id: 2, subject: 'Physics', student: 'Bob Smith', time: 'Tomorrow, 10:00 AM', duration: '1.5 hours' },
        { id: 3, subject: 'Chemistry', student: 'Carol White', time: 'Wed, 3:00 PM', duration: '1 hour' },
    ];

    const recentActivity = [
        { id: 1, action: 'Session completed', student: 'Alice Johnson', time: '2 hours ago' },
        { id: 2, action: 'New booking', student: 'David Brown', time: '5 hours ago' },
        { id: 3, action: 'Review received', rating: 5, time: '1 day ago' },
    ];
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-[#0f3d2e] to-[#1a533f] rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden">
                ...
                <div className="relative">
                    <h1 className="text-4xl font-bold mb-2">
                        Welcome back, {userData?.name || session?.user?.name || 'Tutor'}!
                    </h1>
                            ...
                            {recentActivity.map((activity) => (
                                <div key={activity.id} className="flex items-start space-x-3">
                                    ...
                                    <div>
                                        ...
                                        {'student' in activity && <p className="text-sm text-gray-600">{activity.student}</p>}
                                        {'rating' in activity && (
            ...
            <div className="bg-white rounded-3xl shadow-lg border border-gray-100 p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    >
                        <Award className="h-8 w-8 text-gray-400 group-hover:text-[#0f3d2e] mx-auto mb-2 transition-colors" />
                        <p className="text-sm font-semibold text-gray-700 group-hover:text-[#0f3d2e]">Edit Profile</p>
                    </button>
                    <button className="p-6 border-2 border-dashed border-gray-300 rounded-2xl hover:border-[#0f3d2e] hover:bg-[#d9f99d]/10 transition-all group">
                        ...
                        <p className="text-sm font-semibold text-gray-700 group-hover:text-[#0f3d2e]">Manage Schedule</p>
                    </button>
                    <button className="p-6 border-2 border-dashed border-gray-300 rounded-2xl hover:border-[#0f3d2e] hover:bg-[#d9f99d]/10 transition-all group">
                        <Users className="h-8 w-8 text-gray-400 group-hover:text-[#0f3d2e] mx-auto mb-2 transition-colors" />
                        <p className="text-sm font-semibold text-gray-700 group-hover:text-[#0f3d2e]">View Students</p>
                    </button>
                    <button className="p-6 border-2 border-dashed border-gray-300 rounded-2xl hover:border-[#0f3d2e] hover:bg-[#d9f99d]/10 transition-all group">
                        <DollarSign className="h-8 w-8 text-gray-400 group-hover:text-[#0f3d2e] mx-auto mb-2 transition-colors" />
                        <p className="text-sm font-semibold text-gray-700 group-hover:text-[#0f3d2e]">Earnings</p>
                    </button>
                </div>
            </div>
        </div>
